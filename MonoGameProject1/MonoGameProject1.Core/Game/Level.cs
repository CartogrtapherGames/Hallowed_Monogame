...
using MonoGameProject1.Core.Inputs;
...
namespace MonoGameProject1.Core
{
  ...
  class Level : IDisposable
  {
    ...
    private SettingsManager<MonoGameProject1Leaderboard> settingsManager;
    private bool saved;
    ...
    /// </summary>
    public SettingsManager<MonoGameProject1Leaderboard> LeaderboardManager
    {
      get => settingsManager;

      set
      {
        if (value != null && settingsManager != value)
        {
      ...
      bool readyToPlay = true)
    {
      ...
      if (ReachedExit)
      {
        ...
        if (!saved)
        {
          ...
          if (timeTaken < settingsManager.Settings.FastestTime && gemsCollected == gemsCount)
          {
            newHighScore = true;
          }
    ...
    private void DrawHud(SpriteBatch spriteBatch)
    {
      ...
      if (readyToPlay)
      {
        ...
        Color timeColor = TimeTaken < MaximumTimeToCompleteLevel - WarningTime || ReachedExit || (int)TimeTaken.TotalSeconds % 2 == 0 ? Color.Yellow : Color.Red;
