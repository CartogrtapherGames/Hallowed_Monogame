...
namespace MonoGameProject1.Core.Inputs;
...
public class InputState
{
  ...
  public InputState()
  {
    ...
    else if (MonoGameProject1Game.IsDesktop)
    {
  ...
  public bool IsPauseGame(PlayerIndex? controllingPlayer, Rectangle? rectangle = null)
  {
    ...
    {
      if (rectangle.Value.Contains(CurrentCursorLocation) && (IsLeftMouseButtonClicked() || touchCount > 0))
      {
        pointInRect = true;
      }
    }

    return IsNewKeyPress(Keys.Escape, controllingPlayer, out playerIndex) || IsNewButtonPress(Buttons.Back, controllingPlayer, out playerIndex) || IsNewButtonPress(Buttons.Start, controllingPlayer, out playerIndex) || pointInRect;
  }
  ...
  internal bool IsUIClicked(Rectangle rectangle)
  {
    ...
    if (rectangle.Contains(CurrentCursorLocation) && (IsLeftMouseButtonClicked() || touchCount > 0))
    {
      pointInRect = true;
    }
